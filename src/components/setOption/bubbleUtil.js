...
import bubbleOptions from '@/defaults/options/bubbleData.js'
let echarts = require('echarts/lib/echarts')
export default {
	setOption(attrs, xdata,yDatas) {
		let chartOption = bubbleOptions
		//动画
		chartOption.animation = attrs[7].editable != '2'
		chartOption.animationDuration = attrs[7].properties[0].value
		chartOption.animationDurationUpdate = attrs[7].properties[2].value
		chartOption.animationEasing = attrs[7].properties[1].value
		//全局样式
		...
		//-----r轴----------
		//判断是否按y轴映射
		let isYMap=attrs[4].properties[0].fieldData[0].value
		//根据是否映射，确定最大值最小值
		let rmin=2,rmax=244
		if(isYMap){
			...
			rmin=parseInt(attrs[4].properties[1].value)
			rmax=parseInt(attrs[4].properties[2].value)
		}
		let arrMap=[]
		let rshow=attrs[4].editable != '2' ? true : false
		if(!rshow){
			rmin=2
			rmax=2
		}
		...
		chartOption.legend.textStyle.color = attrs[5].properties[0].fieldData[1].value
		chartOption.legend.textStyle.fontSize = attrs[5].properties[0].fieldData[0].value
		//布局
		//间距
		chartOption.legend.itemGap = parseInt(attrs[5].properties[1].fieldData[0].fieldData[0].value) //左右间距
		chartOption.legend.padding = [parseInt(attrs[5].properties[1].fieldData[0].fieldData[1].value), 5] // 上下间距

		//位置
		let legType = attrs[5].properties[1].fieldData[1].value
		if(legType == '底部居中') {
		...
		chartOption.series = []
		let seriesLength = attrs[6].properties.length
		for(let i = 0; i < seriesLength; i++) {
			chartOption.series.push(JSON.parse(JSON.stringify(iseries)))
		}
		for(let i = 0; i < seriesLength; i++) {
			chartOption.series[i].name = attrs[6].properties[i].fieldData[0].value // 系列名称
			let seitype = attrs[6].properties[i].fieldData[1].value
			if(seitype == '颜色填充') {
				chartOption.series[i].itemStyle.normal.color = attrs[6].properties[i].fieldData[1].fieldData[0].value
			} else {
				let num = attrs[6].properties[i].fieldData[1].fieldData[1].fieldData[2].value
				let lin1 = 0,
					lin2 = 0,
					lin3 = 0,
					lin4 = 0
				...
				let c1 = attrs[6].properties[i].fieldData[1].fieldData[1].fieldData[0].value
				let c2 = attrs[6].properties[i].fieldData[1].fieldData[1].fieldData[1].value
		...
		chartOption.legend.show = attrs[5].editable != '2' ? true : false
		chartOption.animation = attrs[7].editable != '2' ? true : false
