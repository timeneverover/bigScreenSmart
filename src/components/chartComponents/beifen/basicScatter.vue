	...
	export default {
		...
		watch: {
			chartAttr: {
				handler(newValue, oldValue) {　　　
					this.setChartOption()
					if(this.basicScatter != '') {
						this.basicScatter.resize()
					}　
				},
				　deep: true
			}
		...
		mounted() {
			...
			window.addEventListener('resize', () => {
				if(that.basicScatter != '') {
					that.basicScatter.resize()
				}
			})
		},
		methods: {
			initChart() {
				this.basicScatter = this.$echarts.init(document.getElementById(this.chartAttr.attr_id))
			},
			setChartOption() {
				this.initChart()
				...
				this.basicScatter.setOption(options, true)
			},
		}
	}
</script>
