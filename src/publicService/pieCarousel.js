    ...
    function autoShowTip() {
      function showTip() {
        ...
        if (chartType === 'pie' || chartType === 'radar') {
          options.numBox.html(options.realData[dataIndex].value);
          options.nameBox.html(options.realData[dataIndex].name);
          let startValue = 0;
          let endValue = 0;
          if (options.ifRealNum) { //显示真实数值
            if (dataIndex == 0) {
              startValue = 0;
            } else {
              startValue = parseInt(options.realData[dataIndex - 1].value);
            }
            endValue = parseInt(options.realData[dataIndex].value);

          } else {
            if (dataIndex == 0) {
              startValue = 0;
            } else {
              startValue = (parseInt(options.realData[dataIndex - 1].value) / options.numSum).toFixed(4) * 100;
            }
            endValue = (parseInt(options.realData[dataIndex].value) / options.numSum).toFixed(4) * 100;
          }
          let speed = Math.abs(endValue - startValue) / (options.moveTime / 100);
          //不显示真实数值，百分比要算一下
          basicUtil.numRunFun(options.numBox, startValue, endValue, speed, '', 0);
      ...
      showTip();
      timeTicket = setInterval(showTip, options.interval) + options.attrId;
      // lastInterval = timeTicket;
    }
